		...
		do
			...
			do
				...
				then
					for j=1, Targets
					do
						AdjMergeTolleranceMeters[j] = MergeCorrection(TargetDistance[j],LocalAbsRotationalVelocity,MergeTolleranceMeters)--					updates merge tollerance to adjust for manuvering to prevent long range targets from being split by mistake
						if Check(DetectedDist[i],DetectedEle[i],DetectedAzi[i],TargetDistance[j],TargetElevation[j],TargetAzimuth[j],AdjMergeTolleranceMeters[j])
						then
							NewTarget = false
			...
			do
				...
				do
					...
					then--								checks if values duplicated or if any values are nil and if so deletes memory entry to remove duplicate and corrupted target data
						if(i~=j)--							verifies the data is not being read from the same memory location if it is not the same location it then deletes duplicate data
						then
							TargetDistance[i] = nil
							TargetElevation[i] = nil
							TargetAzimuth[i] = nil
						elseif(TargetElevation[i]==nil)--		checks if data is corrupted (currently only elevation) and if it is it deletes the corrupt data to prevent errors
						then
							TargetDistance[i] = nil
							TargetElevation[i] = nil
							TargetAzimuth[i] = nil
